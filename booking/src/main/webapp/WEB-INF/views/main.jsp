		...
		<div class="container">
			...
			<div class="row bg-light pt-4">
				<div class="col-md-6 col-lg-6">
					<c:forEach var="notice" items="${noticeList}" varStatus="status" begin="0" end="5" step="2" >
						<article class="row-4" data-notice-no="${notice.no}">
							<div class="bg-a pull-left"><c:out value="${status.count*2-1}"></c:out></div>
							<h3>${notice.title}</h3>
				...
				<div class="col-md-6 col-lg-6">
					<c:forEach var="notice" items="${noticeList}" varStatus="status" begin="1" end="6" step="2">
						<article class="row-4" data-notice-no="${notice.no}">
							<div class="bg-a pull-left"><c:out value="${status.count*2}"></c:out></div>
							<h3>${notice.title}</h3>
							<p style="height: 100px; overflow: hidden; text-overflow: ellipsis;">${notice.contents}</p>
						</article>
					</c:forEach>
